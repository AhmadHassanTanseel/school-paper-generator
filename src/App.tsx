...
export default function App() {
  ...
  useEffect(() => {
    async function initApp() {
      try {
        ...
        if (res && res.length > 0) {
          setSchoolMeta({
            name: res[0].name || 'Al-Falah Public High School',
            default_session: res[0].default_session || '2025–2026',
            is_setup_complete: Boolean(res[0].is_setup_complete),
            last_backup_date: res[0].last_backup_date || new Date().toISOString().split('T')[0]
          });
        }
      } catch (err) {
  ...
  const daysSinceBackup = Math.floor((new Date().getTime() - new Date(schoolMeta.last_backup_date).getTime()) / (1000 * 3600 * 24));
  ...
  return (
    <div className={`flex h-screen w-screen overflow-hidden bg-[#F8FAFC] font-sans text-slate-800 ${isUrdu ? 'dir-rtl' : ''}`}>
      ...
      {/* FIGMA EXACT LEFT SIDEBAR (#102A43 Deep Slate Navy) */}
      <aside className="w-64 bg-[#102A43] text-slate-300 flex flex-col justify-between shrink-0 border-r border-[#1E3A5F] shadow-xl print:hidden">
        ...
        {/* 9 Navigation Links with Figma Exact Teal Pill (#0D9488) */}
        ...
        <div className="p-4 bg-[#0B1E31] border-t border-slate-700/50 space-y-2.5">
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-800/80 text-[11px]">
            ...
            <div 
              ...
              className="flex items-center gap-1 text-[#34D399] font-bold cursor-pointer hover:underline"
            >
          ...
          <div className="pt-1 text-right">
            <button 
              ...
              className="text-[10px] text-teal-400 hover:underline inline-flex items-center gap-1 font-bold"
            >
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {activeTab === 'history' && <div className="p-8"><OldPapers /></div>}
        {activeTab === 'teachers' && <div className="p-8"><TeacherManagement /></div>}
